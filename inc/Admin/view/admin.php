...
?>
<div class="wrap" id="ud-app" v-click-outside="onClickOutside">
    <h1 class="ud-page-title"> <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
    ...
    <ul class="docs not-loaded ud-masonry" v-sortable>
        <li class="single-doc doc-title" v-for="(doc, index) in docs" :data-id="doc.post.id">
            <h3>
                ...
                <a v-if="doc.post.caps.edit" target="_blank" :href="editurl + doc.post.id">
                    ...
                    <span class="ud-title">{{ doc.post.title }}</span>
                ...
                <span class="ud-row-actions">
                            ...
                            <ul class="action-sub-menu">
                                <li>
                                    <a target="_blank" :href="viewurl + doc.post.id"
                                        title="<?php esc_attr_e( 'View the doc', 'ud' );?>">
                                        <?php esc_attr_e( 'View', 'ud' );?> </a>
                                </li>
                                <li>
                                    <a target="_blank" href="#" v-on:click.prevent="quickEdit(doc, $event)"
                                        title="<?php esc_attr_e( 'Wuick Edit the doc', 'ud' );?>">
                                        <?php esc_attr_e( 'Quick Edit', 'ud' );?> </a>
                                </li>
                                <li>
                                    <a target="_blank" :href="editurl + doc.post.id"
                                        title="<?php esc_attr_e( 'Edit the doc', 'ud' );?>">
                                        <?php esc_attr_e( 'Edit', 'ud' );?> </a>
                                </li>
                                <li>
                                    <a href="" v-on:click="copyDoc(doc,$event)"
                                        class="copy"><?php esc_html_e( 'Duplicate', 'ud' )?></a>
                                </li>
                                <?php if('condition' == $ia_doc_show_type): ?>
                                <li>
                                    <a href="" v-on:click.prevent="showCondition(doc,$event)"
                                        class="copy"><?php esc_html_e( 'Conditions', 'ud' )?></a>
                                </li>

                                <?php endif; ?>
                                <li>
                                    <span v-if="doc.post.caps.delete" class="ud-btn-remove"
                                        v-on:click="removeDoc(index, docs)"
                                        title="<?php esc_attr_e( 'Delete this doc', 'ud' );?>"><?php esc_html_e( 'Delete', 'ud' )?></span>
                    ...
                    <!-- <span class="ud-btn-reorder"><span class="dashicons dashicons-menu"></span></span> -->
                </span>
            </h3>
                    ...
                    <li v-for="(section, index) in doc.child" :data-id="section.post.id" class="doc-title">
                        <span class="section-title" v-on:click="toggleCollapse">
                            <a v-if="section.post.caps.edit" target="_blank" :href="editurl + section.post.id">
                                ...
                                <span class="ud-title">{{ section.post.title }}</span>
                                ...
                                <ul class="actions-menu">
                                    <li>
                                        ...
                                        <ul class="action-sub-menu">
                                            ...
                                            <li>
                                                <span v-if="section.post.caps.delete" class="ud-btn-remove"
                            ...
                            <li class="article doc-title" v-for="(article, index) in section.child"
                                        ...
                                        <ul class="actions-menu">
                                            <li>
                                                ...
                                                <ul class="action-sub-menu">
                                                    ...
                                                    <li>
                                                        <span v-if="article.post.caps.delete" class="ud-btn-remove"
                                ...
                                <ul class="articles" v-if="article.child.length">
                                    <li v-for="(art, index) in article.child">
                                        ...
                                        <span class="actions ud-row-actions">
                                            <a target="_blank" :href="viewurl + article.post.id"
                                                title="<?php esc_attr_e( 'Preview the article', 'ud' );?>"><span
                                                    class="dashicons dashicons-external"></span></a>
                                            <span class="ud-btn-remove" v-if="art.post.caps.delete"
                                                v-on:click="removeArticle(index, article.child)"
                                                title="<?php esc_attr_e( 'Delete this article', 'ud' );?>"><span
                                                    class="dashicons dashicons-trash"></span></span>
                                            <span class="ud-btn-add" v-on:click.prevent="addSection(doc)"
                                                title="<?php esc_attr_e( 'Add article', 'ud' );?>"><span
            ...
            <div class="add-section">
                <a class="add-section-btn" href="#" v-on:click.prevent="addSection(doc)">
                    ...
                    <?php _e( 'Add Section', 'ud' );?>
                </a>
            </div>
        </li>
    </ul>

    <div class="ud-no-docs not-loaded" v-show="!docs.length">
        ...
        <h4><?php esc_html_e( 'No docs has been found', 'ud' ) ?></h4>
        <p><?php esc_html_e( 'You don’t have any doc yet, but you can always create a new one.', 'ud' ) ?></p>
        <a class="fd-doc-page-title" href="#" v-on:click.prevent="addDoc"><svg width="12" height="12"
                viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- <?php printf('<h4>' . __( 'No docs has been found. Perhaps %s?', 'ud' ). 'h', '<a href="#" v-on:click.prevent="addDoc">' . __( 'create one', 'ud' ) . '</a>' );?> -->
    </div>
